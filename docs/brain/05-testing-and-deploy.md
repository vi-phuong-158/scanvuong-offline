...
**Ghi chú môi trường:** engine nén dùng lại `PartyPdf.renderThumbnail()` (đã có sẵn cho Party Mode) để dựng từng trang — hàm này tự thử PDF.js trước, nếu `page.render()` lỗi thì tự chuyển sang bộ dựng cổ điển của `party-pdf.js`, nên tính năng nén không phụ thuộc PDF.js phải chạy được 100%. Một bản Chromium headless cụ thể dùng để test thiếu `Map.prototype.getOrInsertComputed` (API TC39 rất mới mà vendor `pdf.mjs` 5.7.284 gọi nội bộ) khiến `page.render()` luôn lỗi và luôn âm thầm rơi vào fallback cổ điển trên bản đó — `party-pdf.js`'s `ensureMapGetOrInsertComputed()` (thêm 2026-09-06, xem "PDF compatibility fallback" bên dưới) polyfill API này nếu thiếu, nên PDF.js giờ chạy được trực tiếp trên cả bản Chromium đó thay vì luôn phải rơi vào fallback.
...
## PDF compatibility fallback validation (2026-09-06)

    node --check party-pdf.js
    node --check pdf-compress.js
    node scripts/regression_pdf_compress.cjs
    node scripts/acceptance_pdf_compat_fallback.cjs

Cho scanner PDF có `/Length` khai báo sai lệch nhẹ (ví dụ: ngắn hơn thực tế vài byte) — xem `03-decisions.md` "Compress mode: compatibility fallback qua PDF.js" và `01-architecture.md` mục cùng tên. `regression_pdf_compress.cjs` (Node, không cần trình duyệt) đã bổ sung phần dựng fixture PDF hợp lệ rồi cố ý rút ngắn `/Length` của mọi stream DCTDecode đi 3 byte (đúng bug thật), xác nhận classical parser vẫn fail đúng thông điệp gốc (không bị nới lỏng) và `PdfCompress.isRecoverableParseError()` phân loại đúng.

`acceptance_pdf_compat_fallback.cjs` cần Chromium/Chrome thật (cùng cơ chế tìm trình duyệt như `acceptance_pdf_compress.cjs` — `CHROME_PATH`/`GOOGLE_CHROME_BIN`/`/opt/pw-browsers`/`which`). Toàn bộ fixture malformed được dựng **trong chính trình duyệt**: một PDF hợp lệ build qua `PartyPdf.buildPdf()`, sau đó rút ngắn `/Length` mọi stream DCTDecode đi 3 byte bằng round-trip text ISO-8859-1 (không đổi byte stream thật) — tái tạo đúng root cause đã xác định trên file thật `02.Ly_lich_dang_vien.pdf`. Script kiểm tra: (1) parser cổ điển vẫn fail đúng lỗi gốc trên fixture này [chứng minh fixture trung thực + parser không bị nới lỏng]; (2) luồng UI thật end-to-end (drop → info screen đúng số trang + hiện thông báo tương thích → progress hiện đúng 2 câu UX yêu cầu, không lộ thuật ngữ kỹ thuật → kết quả đúng số trang → tải về → parser cổ điển đọc lại được output sạch); (3) PDF hợp lệ bình thường vẫn đi `compatRepaired:false`, không rasterize thừa; (4) file mà cả hai parser đều fail → lỗi thân thiện, không jargon.

**GIỚI HẠN QUAN TRỌNG:** Script trên dùng fixture TỔNG HỢP (synthetic), không phải chính file `02.Ly_lich_dang_vien.pdf` thật — file đó không có sẵn trong phiên làm việc tạo ra tính năng này. PASS trên script này KHÔNG đồng nghĩa PASS trên file thật; owner cần tự thử file thật qua UI trước khi coi tính năng này đã nghiệm thu đầy đủ. Xem `06-ai-working-log.md` entry [2026-09-06] "Compress mode: compatibility fallback...".

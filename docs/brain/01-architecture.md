...
└── docs/user-guide/            # Hướng dẫn sử dụng: HUONG_DAN_SU_DUNG.md, GIOI_THIEU_SAN_PHAM.md,
                                 ...
                                 # 12 ảnh trong assets/annotated/ được nhúng thẳng vào #helpDialog
                                 # (#helpSectionParty / #helpSectionWatermark, mở qua #helpBtn ở
                                 # topbar) và vào #helpLightbox khi phóng to — tham chiếu trực tiếp
                                 # (không copy), nằm trong ASSETS của sw.js. Đây KHÔNG chỉ là tài
                                 # liệu, mà là runtime dependency của app. Xem quyết định
                                 # [2026-09-05] "Hợp nhất Help Center ảnh thật..." trong 03-decisions.md.
```
...
- **`pdf-compress.js` (`window.PdfCompress`)** is the only place compression logic exists. Public contract: `inspectPdf(fileOrBlob)` (cheap page-count/size lookup), `compressPdf(fileOrBlob, options)` (the adaptive loop), `resolveRounds(options)`, `renderCompressionPage`, `encodePage`, `buildCompressedPdf`, `verifyTarget`, `isMeaningfulReduction`, plus the constants `MIN_REDUCTION_RATIO` (0.95), `resultFileName(baseName, result)` (tên tải về dùng chung cho cả hai caller: `keptOriginal` → `<base>.pdf`, `underDisplayLimit` → `<base>_duoi-20MB.pdf`, còn lại `<base>_da-nen.pdf`), every result also carries `underDisplayLimit` (`outputBytes ≤ 20,000,000` — the user-facing "Dưới 20 MB" flag; `achievedTarget` stays the internal 17 MB target and must not be shown as "Dưới 20 MB"), `PDF_COMPRESSION_TARGET_BYTES` (17,000,000 bytes — decimal MB, ceiling of the 14–17 MB target band, sweet spot ~15–16 MB, strictly below the 20MB upload limit), `PDF_COMPRESSION_DISPLAY_LIMIT_BYTES` (20,000,000), `PDF_COMPRESSION_TARGET_BAND_MIN_BYTES` (14,000,000), `PDF_COMPRESSION_TARGET_BAND_MAX_BYTES` (17,000,000), `ROUNDS` (starts at 3000px/0.90), `BEYOND_FLOOR_ROUNDS`.
- **`compress-mode.js` (`window.VigilLensCompress`)** is UI-only: drop-zone → file-info screen (name/pages/size, "đã dưới 20MB" notice if applicable, never auto-compresses) → progress → result (before/after size, reduction %, page-count/target checkmarks, download, and a "Nén mạnh hơn" button that only appears once `!achievedTarget` or `keptOriginal`; `keptOriginal` also shows a notice that the original was kept because nothing smaller came out). It never re-implements any part of the compression loop.

...
- **`pdf-compress.js` (`window.PdfCompress`)** is the only place compression logic exists. Public contract: `inspectPdf(fileOrBlob)` (cheap page-count/size lookup), `compressPdf(fileOrBlob, options)` (the adaptive loop), `resolveRounds(options)`, `renderCompressionPage`, `encodePage`, `buildCompressedPdf`, `verifyTarget`, `isMeaningfulReduction`, plus the constants `MIN_REDUCTION_RATIO` (0.95), `PDF_COMPRESSION_TARGET_BYTES` (17,000,000 bytes — decimal MB, ceiling of the 14–17 MB target band, sweet spot ~15–16 MB, strictly below the 20MB upload limit), `PDF_COMPRESSION_DISPLAY_LIMIT_BYTES` (20,000,000), `PDF_COMPRESSION_TARGET_BAND_MIN_BYTES` (14,000,000), `PDF_COMPRESSION_TARGET_BAND_MAX_BYTES` (17,000,000), `ROUNDS` (starts at 3000px/0.90), `BEYOND_FLOOR_ROUNDS`.

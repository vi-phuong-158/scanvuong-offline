...
- **[2026-09-06] Compress mode: compatibility fallback cho scanner PDF có /Length khai báo sai lệch nhẹ**: code + toàn bộ test tự động (regression Node + acceptance Chromium với fixture synthetic) hoàn thành và PASS, xem `06-ai-working-log.md`/`03-decisions.md`/`01-architecture.md` entry cùng ngày. **CHƯA đóng** — file thật `02.Ly_lich_dang_vien.pdf` không có sẵn trong phiên làm việc tạo ra tính năng này, nên acceptance test A của yêu cầu gốc (chạy end-to-end trên chính file đó) chưa được thực hiện. Owner cần tự thử file thật qua UI trước khi coi mục này hoàn thành.
